...
class Auth extends BaseController
{
	...
	protected $viewsFolder = 'App\Views\backend';
	...
	public function login()
	{
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			if ($this->ionAuth->login($this->request->getVar('identity'), $this->request->getVar('password'), $remember)) {
				...
				$this->session->setFlashdata('errors', $this->ionAuth->errors($this->validationListTemplate));
	...
	public function activate_register(int $id, string $code = ''): \CodeIgniter\HTTP\RedirectResponse
	{
		$activation = false;
	...
	{
		$delete = $this->ionAuth->deleteUser($id);
	...
	{
		return $this->ionAuth->group()->row()->name;
	}
